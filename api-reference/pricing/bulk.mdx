...
<RequestExample>
```bash cURL
curl -X POST https://api.chidori.africa/enterprise-service/api/pricing/bulk \
...
const response = await fetch('https://api.chidori.africa/enterprise-service/api/pricing/bulk', {
  method: 'POST',
  headers: {
...
response = requests.post(
    'https://api.chidori.africa/enterprise-service/api/pricing/bulk',
    headers={
        'X-API-Key': 'sk_live_xxx.yyy',
        'Content-Type': 'application/json'
    },
...
{
  "status": false,
  "message": "Each stop must contain lat and lon numbers"
}
```

```json 422 Out of service area
{
  "status": false,
  "reason": "OUT_OF_SERVICE_AREA",
  "message": "We're temporarily not delivering to this area yet."
}
```
</ResponseExample>
...
|--------|---------|-------------|
| `400` | Validation error | A required field is missing or malformed, or `stops` exceeds 50 |
| `401` | Unauthorized | Invalid or missing API key |
| `403` | Forbidden | API key lacks `pricing:read` permission |
| `422` | Out of service area | The pickup or any stop falls outside the bike service area — the whole quote fails. Carries `reason: "OUT_OF_SERVICE_AREA"`. |
...
```bash
curl -X POST https://api.chidori.africa/enterprise-service/api/deliveries/create-bulk \
  -H "X-API-Key: sk_live_xxx.yyy" \
  -H "Content-Type: application/json" \
